...
Usage (drop-in replacement for FA3):
    from berserker.flash_attention import flash_attn
...
def _resolve_use_fa3():
    ...
    if HAS_FA3:
        ...
        from berserker.common import COMPUTE_DTYPE
...
def _expand_kv_for_gqa(q, k, v):
    """Expand K/V heads to match Q heads for GQA, compatible with all PyTorch 2.x versions."""
...
def _sdpa_attention(q, k, v, window_size):
    ...
    # Expand K/V heads to match Q for GQA (works on all PyTorch 2.x, not just 2.5+)
...
def flash_attn_func(q, k, v, causal=False, window_size=(-1, -1)):
    ...
    # .contiguous() ensures the Memory Efficient SDPA backend is used on T4 (avoids Math backend fallback)
...
def flash_attn_with_kvcache(q, k_cache, v_cache, k=None, v=None, cache_seqlens=None,
    ...
    # .contiguous() ensures the Memory Efficient SDPA backend is used on T4
