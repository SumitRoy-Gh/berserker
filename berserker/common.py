"""
Common utilities for berserker.
"""
...
# Override with BERSERKER_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("BERSERKER_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via BERSERKER_DTYPE={env}"
    if torch.cuda.is_available():
        ...
        # fp16 training with GradScaler is supported (see base_train.py)
        # T4 (SM 75) has excellent fp16 tensor cores: 65 TFLOPS fp16 vs 8.1 TFLOPS fp32
        return torch.float16, f"auto-detected: CUDA SM {capability[0]}{capability[1]} (pre-Ampere, using fp16 with GradScaler)"
    # Note: MPS on recent macOS also handles bf16 fine, opt in via BERSERKER_DTYPE=bfloat16
...
def get_base_dir():
    # co-locate berserker intermediates with other cached data in ~/.cache (by default)
    if os.environ.get("BERSERKER_BASE_DIR"):
        berserker_dir = os.environ.get("BERSERKER_BASE_DIR")
    else:
        home_dir = os.path.expanduser("~")
        cache_dir = os.path.join(home_dir, ".cache")
        berserker_dir = os.path.join(cache_dir, "berserker")
    os.makedirs(berserker_dir, exist_ok=True)
    return berserker_dir
...
def print_banner():
    # Sharp anime/fantasy style font for BERSERKER
    banner = """
 ██████╗ ███████╗██████╗ ███████╗███████╗██████╗ ██╗  ██╗███████╗██████╗ 
 ██╔══██╗██╔════╝██╔══██╗██╔════╝██╔════╝██╔══██╗██║ ██╔╝██╔════╝██╔══██╗
 ██████╔╝█████╗  ██████╔╝███████╗█████╗  ██████╔╝█████╔╝ █████╗  ██████╔╝
...
# and PR: https://github.com/karpathy/berserker/pull/147
def get_peak_flops(device_name: str) -> float:
    ...
    _PEAK_FLOPS_TABLE = (
        ...
        (["4090"], 165.2e12),
        (["3090"], 71e12),
        # NVIDIA Turing (fp16 tensor cores)
        (["t4"], 65e12),
    )
